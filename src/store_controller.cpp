...
public:
                     ...
                     distance_goal_reached(false), all_goals_reached(false)
  {
   ...
   // Open CSV file for writing
    ...
    csv_file << "Distance,Angle,Output Array\n";
...
{
  std::stringstream output_array_stream;

  // Log initial distance and angle
  output_array_stream << initial_distance << "," << initial_angle << ";";

  // Log series of cmd_vel values
  for (const auto& vel : cmd_vel_values)
  {
    output_array_stream << vel.first << "," << vel.second << ";";
  }

  std::string output_array = output_array_stream.str();

  // Write data to CSV file
  csv_file << distance << "," << angle << "," << output_array << "\n";
  ...
  void goToGoal()
  {
      ...
      if (distance_goal_reached)
	{
      ...
      {
            // Store initial distance and angle when the goal starts
